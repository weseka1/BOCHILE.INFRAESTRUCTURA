// Fix del jsonBody en "Responder al Cliente respond.io":
// El parser expone el campo como "channel_id" (no "channel_id_val").
// Mi script 72 puso channel_id_val por error -> caia siempre al fallback 505934.
...
const NEW_JSON_BODY = `={{ JSON.stringify({ channelId: ({'506217': 505934, '508111': 508111}[String($('Parsear Mensaje').item.json.channel_id || '')] || 505934), message: { type: 'text', text: $('Vendedor CORE').item.json.output } }) }}`;
...
(async () => {
  ...
  fs.writeFileSync(
    path.join(bkpDir, `${WF}_pre_fix_channel_id_${new Date().toISOString().replace(/[:.]/g, '-')}.json`),
  ...
  if (responder.parameters.jsonBody === NEW_JSON_BODY) {
    console.log('ℹ️  Ya estaba aplicado');
    return;
  }

  responder.parameters.jsonBody = NEW_JSON_BODY;
  console.log('Nuevo body:', NEW_JSON_BODY.slice(0, 250));
  ...
  console.log('✅ channelId ahora lee channel_id (correcto). Ventas 506217 -> 505934, Yamil 508111 -> 508111.');
