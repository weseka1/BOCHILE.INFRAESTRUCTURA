...
import { Users, Home, Calendar, TrendingUp, DollarSign, Sparkles } from 'lucide-react';
...
export function VentasPage() {
  ...
  const { data: visitas, isLoading: l3 } = useVisitas();
  ...
  return (
    <>
      <PageHeader title="Dashboard Ventas" subtitle="KPIs e indicadores del area de venta de propiedades" />

      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        <StatCard label="Leads de venta" value={stats.leadsTotal} icon={Users} accent="blue" />
        ...
        <StatCard label="Propiedades en venta" value={stats.propsTotal} icon={Home} accent="gold" />
      ...
      <Card>
        <h3 className="text-sm uppercase tracking-wider text-text-muted mb-3">Leads de venta recientes</h3>
        <div className="space-y-2">
          {(leads ?? []).filter((l:Lead) => isVenta(l.operacion)).slice(0, 10).map((l:Lead) => (
          ...
          {(leads ?? []).filter((l:Lead) => isVenta(l.operacion)).length === 0 && (
            <p className="text-text-muted text-sm text-center py-4">Sin leads de venta todavia.</p>
          )}
        </div>
      </Card>
    </>
