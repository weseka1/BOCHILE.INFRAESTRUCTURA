/**
 * BOCHILE · SISTEMA OPERATIVO · BUILD FINAL
 *
 * NUKE_AND_REBUILD: borra TODAS las hojas del workbook y lo
 * reconstruye desde cero. Garantiza 17 pestanas exactas, sin
 ...
 * 4. Ctrl+S. Run -> NUKE_AND_REBUILD. Aprobar permisos.
 * 5. Esperar 60-90 seg. F5 en la pestana del spreadsheet.
 *
 * Resultado: 8 hojas base con seed data + 8 hojas analiticas
 * con KPIs y formulas + 1 CONFIG. Total 17.
 */
...
const F = { title: 'Playfair Display', body: 'Roboto', mono: 'Roboto Mono' };
...
function NUKE_AND_REBUILD() {
  ...
  Logger.log('NUKE: borrando todas las hojas existentes...');
  nukeAllSheets(ss);

  Logger.log('REBUILD: creando 8 hojas base con seed...');
  Object.keys(BASE_SHEETS).forEach(function(name){
    buildBaseSheet(ss, name, BASE_SHEETS[name], SEED[name] || []);
  });

  Logger.log('REBUILD: creando 8 hojas analiticas + CONFIG...');
  buildPortada(ss);
  buildAgenda(ss);
  buildPipeline(ss);
  buildEmbudo(ss);
  ...
  Logger.log('REBUILD: borrando placeholder y reordenando...');
  // Borrar la hoja temporal que NUKE dejo
  const sheets = ss.getSheets();
  for (let i = 0; i < sheets.length; i++) {
    const nm = sheets[i].getName();
    if (nm.indexOf('__tmp__') === 0) {
      try { ss.deleteSheet(sheets[i]); } catch(e){}
    }
  }
  reorderTabs(ss, SHEET_ORDER);

  ss.setActiveSheet(ss.getSheetByName('Portada'));
  SpreadsheetApp.flush();
  Logger.log('LISTO. Refresca el spreadsheet (F5).');
}

function nukeAllSheets(ss) {
  // Crear hoja temporal primero (Google no permite borrar todas)
  const tempName = '__tmp__' + Date.now();
  ss.insertSheet(tempName);
  const sheets = ss.getSheets();
  for (let i = 0; i < sheets.length; i++) {
    if (sheets[i].getName() !== tempName) {
      try { ss.deleteSheet(sheets[i]); } catch(e){}
    }
  }
}
...
function clearSheet(s) {
  ...
  s.setRowHeights(1, Math.min(s.getMaxRows(), 300), 21);
}
...
function placeTitle(s, row, col, span, title, subtitle) {
  s.getRange(row, col, 1, span).merge()
   .setValue(title).setFontFamily(F.title).setFontSize(28).setFontWeight('bold').setFontColor(C.navy)
   .setHorizontalAlignment('left').setVerticalAlignment('bottom').setBackground(C.bgPaper);
  s.setRowHeight(row, 48);
  if (subtitle) {
    s.getRange(row + 1, col, 1, span).merge()
     .setValue(subtitle.toUpperCase()).setFontFamily(F.body).setFontSize(9).setFontWeight('bold').setFontColor(C.gold)
     .setHorizontalAlignment('left').setVerticalAlignment('top').setBackground(C.bgPaper);
    s.setRowHeight(row + 1, 18);
  }
  const lineRow = subtitle ? row + 2 : row + 1;
  s.getRange(lineRow, col, 1, span).merge().setBackground(C.gold);
...
function placeDateLabel(s, row, col, span) {
  s.getRange(row, col, 1, span).merge()
   ...
   .setFontFamily(F.body).setFontSize(10).setFontColor(C.textSoft).setFontStyle('italic')
   .setHorizontalAlignment('left').setVerticalAlignment('middle').setBackground(C.bgPaper);
  s.setRowHeight(row, 24);
}

function placeKPI(s, startRow, startCol, span, label, formula, format, accent) {
  const card = s.getRange(startRow, startCol, 3, span);
  card.setBackground(C.bg);
  card.setBorder(true, true, true, true, false, false, C.lineStrong, SpreadsheetApp.BorderStyle.SOLID);
  ...
  labelR.setHorizontalAlignment('left').setVerticalAlignment('middle').setBackground(C.bg);
  s.setRowHeight(startRow, 30);

  const valueR = s.getRange(startRow + 1, startCol, 1, span);
  valueR.merge();
  valueR.setFormula(formula);
  if (format) valueR.setNumberFormat(format);
  valueR.setFontFamily(F.title).setFontSize(32).setFontWeight('bold').setFontColor(C.navy);
  valueR.setHorizontalAlignment('left').setVerticalAlignment('middle').setBackground(C.bg);
  s.setRowHeight(startRow + 1, 56);

  const accentR = s.getRange(startRow + 2, startCol, 1, span);
  accentR.merge().setBackground(accent || C.gold);
...
function placeSectionLabel(s, row, col, span, text) {
  s.getRange(row, col, 1, span).merge()
   .setValue(text.toUpperCase()).setFontFamily(F.body).setFontSize(9).setFontWeight('bold').setFontColor(C.gold)
   .setHorizontalAlignment('left').setVerticalAlignment('middle').setBackground(C.bgPaper);
  s.setRowHeight(row, 24);
  s.getRange(row, col, 1, span).setBorder(null, null, true, null, false, false, C.lineStrong, SpreadsheetApp.BorderStyle.SOLID);
}

function placeSpacer(s, row, height) { s.setRowHeight(row, height || 16); }
...
function buildBaseSheet(ss, name, headers, seedRows) {
  ...
  if (seedRows && seedRows.length > 0) {
    ...
    s.getRange(2, 1, seedRows.length, headers.length)
     .setFontFamily(F.body).setFontSize(10).setFontColor(C.textMain).setVerticalAlignment('middle');
    for (let i = 0; i < seedRows.length; i++) s.setRowHeight(2 + i, 28);
  }
...
function buildPortada(ss) {
  ...
  s.setColumnWidth(1, 36);
  [2,3,4].forEach(function(c){ s.setColumnWidth(c, 100); });
  s.setColumnWidth(5, 20);
  [6,7,8].forEach(function(c){ s.setColumnWidth(c, 100); });
  s.setColumnWidth(9, 20);
  [10,11,12].forEach(function(c){ s.setColumnWidth(c, 100); });
  s.setColumnWidth(13, 36);

  paintCanvas(s, 14, 80);
  placeSpacer(s, 1, 28);

  let cursor = placeTitle(s, 2, 2, 11, 'Bochile Inmobiliaria', 'Bahia Blanca · Desde 1970 · Sistema Operativo');
  placeSpacer(s, cursor, 14); cursor += 1;
  placeDateLabel(s, cursor, 2, 11); cursor += 1;
  placeSpacer(s, cursor, 32); cursor += 1;

  placeSectionLabel(s, cursor, 2, 11, 'Indicadores comerciales'); cursor += 1;
  ...
  cursor += 3;
  placeSpacer(s, cursor, 28); cursor += 1;

  placeSectionLabel(s, cursor, 2, 11, 'Alquileres y servicio'); cursor += 1;
  placeSpacer(s, cursor, 10); cursor += 1;
  ...
  placeSectionLabel(s, cursor, 2, 11, 'Accesos rapidos'); cursor += 1;
  placeSpacer(s, cursor, 8); cursor += 1;
  ...
  for (let i = 0; i < accesos.length; i++) {
    ...
    s.getRange(row, 2, 1, 4).merge()
     ...
     .setHorizontalAlignment('left').setVerticalAlignment('middle').setBackground(C.bg);
    s.getRange(row, 6, 1, 7).merge()
     .setValue(accesos[i][1]).setFontFamily(F.body).setFontSize(10).setFontColor(C.textSoft)
     .setHorizontalAlignment('left').setVerticalAlignment('middle').setBackground(C.bg);
  ...
  s.getRange(cursor, 2, 1, 11).merge()
   ...
   .setFontFamily(F.body).setFontSize(9).setFontColor(C.textMuted).setFontStyle('italic')
   .setHorizontalAlignment('left').setVerticalAlignment('middle').setBackground(C.bgPaper);
  s.setRowHeight(cursor, 28);
}
...
function buildAgenda(ss) {
  ...
  clearSheet(s); s.setTabColor(C.gold);
  s.setColumnWidth(1, 36); s.setColumnWidth(2, 80); s.setColumnWidth(3, 200);
  s.setColumnWidth(4, 170); s.setColumnWidth(5, 280); s.setColumnWidth(6, 140);
  s.setColumnWidth(7, 320); s.setColumnWidth(8, 36);
  paintCanvas(s, 8, 100);
  placeSpacer(s, 1, 28);
  let cursor = placeTitle(s, 2, 2, 6, 'Agenda de visitas', 'Hoy y proximos 7 dias');
  placeSpacer(s, cursor, 20); cursor += 1;
  placeTableHeader(s, cursor, 2, ['Hora','Cliente','Vendedor','Direccion','Estado','Observaciones']);
  const headerRow = cursor; cursor += 1;
  s.getRange(cursor, 2).setFormula(
    '=IFERROR(QUERY(visitas!A:O,"select I, F, E, G, J, M where H >= date \'"&TEXT(TODAY(),"yyyy-mm-dd")&"\' and H <= date \'"&TEXT(TODAY()+7,"yyyy-mm-dd")&"\' order by H asc, I asc",0),"")'
  );
  s.getRange(cursor, 2, 50, 6).setFontFamily(F.body).setFontSize(11).setFontColor(C.textMain).setVerticalAlignment('middle').setBackground(C.bg);
...
function buildPipeline(ss) {
  ...
  s.setColumnWidth(1, 36);
  for (let i = 0; i < etapas.length; i++) s.setColumnWidth(2 + i, 210);
  s.setColumnWidth(8, 36);
  paintCanvas(s, 8, 200);
  placeSpacer(s, 1, 28);
  let cursor = placeTitle(s, 2, 2, 6, 'Pipeline comercial', 'Kanban por etapa · score 0-100 a la derecha del nombre');
  placeSpacer(s, cursor, 20); cursor += 1;
  for (let i = 0; i < etapas.length; i++) {
    s.getRange(cursor, 2 + i).setValue(etapas[i].toUpperCase())
     .setBackground(C.navy).setFontColor(C.textOnDark).setFontWeight('bold')
     .setFontFamily(F.body).setFontSize(9).setHorizontalAlignment('center').setVerticalAlignment('middle');
  }
  s.setRowHeight(cursor, 34);
  ...
  s.setRowHeight(countRow, 22); s.setFrozenRows(countRow);
  cursor = countRow + 1;
  placeSpacer(s, cursor, 8); cursor += 1;
  for (let i = 0; i < etapas.length; i++) {
  ...
  s.getRange(cursor, 2, 100, etapas.length).setBackground(C.bg).setFontColor(C.textMain);
  for (let r = cursor; r < cursor + 100; r++) s.setRowHeight(r, 30);
...
function buildEmbudo(ss) {
  ...
  clearSheet(s); s.setTabColor(C.gold);
  s.setColumnWidth(1, 36); s.setColumnWidth(2, 280); s.setColumnWidth(3, 140);
  s.setColumnWidth(4, 180); s.setColumnWidth(5, 240); s.setColumnWidth(6, 36);
  paintCanvas(s, 6, 60);
  placeSpacer(s, 1, 28);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    s.getRange(r, 3).setFormula(rows[i][1]).setFontFamily(F.title).setFontSize(22).setFontWeight('bold').setFontColor(C.navy).setHorizontalAlignment('left').setVerticalAlignment('middle');
    if (i > 0) s.getRange(r, 4).setFormula('=IFERROR(C' + r + '/C' + (r - 1) + ',0)').setNumberFormat('0.0%').setFontFamily(F.body).setFontSize(11).setFontColor(C.gold).setFontWeight('bold').setVerticalAlignment('middle');
    else s.getRange(r, 4).setValue('—').setFontColor(C.textMuted).setHorizontalAlignment('center').setVerticalAlignment('middle');
    s.getRange(r, 5).setFormula('=REPT(CHAR(9608),ROUND(C' + r + '/MAX($C$' + cursor + ':$C$' + (cursor + rows.length - 1) + ')*22,0))')
...
function buildRanking(ss) {
  ...
  clearSheet(s); s.setTabColor(C.gold);
  s.setColumnWidth(1, 36); s.setColumnWidth(2, 60); s.setColumnWidth(3, 240);
  s.setColumnWidth(4, 130); s.setColumnWidth(5, 110); s.setColumnWidth(6, 180); s.setColumnWidth(7, 36);
  paintCanvas(s, 7, 60);
  placeSpacer(s, 1, 28);
...
function buildDemanda(ss) {
  ...
  clearSheet(s); s.setTabColor(C.gold);
  s.setColumnWidth(1, 36); s.setColumnWidth(2, 240); s.setColumnWidth(3, 130); s.setColumnWidth(4, 300); s.setColumnWidth(5, 36);
  paintCanvas(s, 5, 60);
  placeSpacer(s, 1, 28);
  let cursor = placeTitle(s, 2, 2, 3, 'Demanda por barrio', 'Donde estan buscando los clientes');
  placeSpacer(s, cursor, 24); cursor += 1;
  ...
  for (let i = 0; i < 20; i++) {
    ...
    s.getRange(r, 4).setFormula('=IF(C' + r + '<>"",REPT(CHAR(9608),ROUND(C' + r + '/MAX($C$' + cursor + ':$C$' + (cursor + 19) + ')*28,0)),"")')
...
function buildAlquileres(ss) {
  ...
  paintCanvas(s, 14, 80);
  placeSpacer(s, 1, 28);
  let cursor = placeTitle(s, 2, 2, 11, 'Alquileres y cobranza', 'Contratos activos, vencimientos y atrasos');
  placeSpacer(s, cursor, 24); cursor += 1;
  ...
  placeSectionLabel(s, cursor, 2, 11, 'Detalle por contrato'); cursor += 1;
  placeSpacer(s, cursor, 10); cursor += 1;

  s.setColumnWidth(2, 80); s.setColumnWidth(3, 220); s.setColumnWidth(4, 200);
  s.setColumnWidth(5, 110); s.setColumnWidth(6, 80); s.setColumnWidth(7, 100);
  s.setColumnWidth(8, 100); s.setColumnWidth(9, 130);
  ...
  for (let i = 0; i < 30; i++) {
    const r = cursor + i;
    s.getRange(r, 2, 1, 8).setFontFamily(F.body).setFontSize(11).setFontColor(C.textMain).setVerticalAlignment('middle').setBackground(C.bg);
...
function buildFeed(ss) {
  ...
  clearSheet(s); s.setTabColor(C.gold);
  s.setColumnWidth(1, 36); s.setColumnWidth(2, 170); s.setColumnWidth(3, 190);
  s.setColumnWidth(4, 200); s.setColumnWidth(5, 380); s.setColumnWidth(6, 130);
  s.setColumnWidth(7, 130); s.setColumnWidth(8, 36);
  paintCanvas(s, 8, 200);
  placeSpacer(s, 1, 28);
  let cursor = placeTitle(s, 2, 2, 6, 'Feed de actividad', 'Ultimas 100 acciones de la IA · mas reciente arriba');
  placeSpacer(s, cursor, 24); cursor += 1;
  ...
  for (let i = 0; i < 100; i++) {
    const r = cursor + i;
    s.getRange(r, 2, 1, 6).setFontFamily(F.body).setFontSize(11).setFontColor(C.textMain).setVerticalAlignment('middle').setBackground(C.bg);
    s.getRange(r, 2, 1, 6).setBorder(null, null, true, null, false, false, C.lineHair, SpreadsheetApp.BorderStyle.SOLID);
    s.setRowHeight(r, 32);
  }
  s.setFrozenRows(cursor - 1);
...
function buildConfig(ss) {
  ...
  clearSheet(s); s.setTabColor(C.textMuted);
  s.setColumnWidth(1, 36);
  [2,3,4,5].forEach(function(c){ s.setColumnWidth(c, 180); });
  s.setColumnWidth(6, 20); s.setColumnWidth(7, 220); s.setColumnWidth(8, 260); s.setColumnWidth(9, 36);
  paintCanvas(s, 10, 50);
  placeSpacer(s, 1, 28);
  ...
  placeTableHeader(s, cursor, 7, ['Parametro','Valor']);
  cursor += 1;
  ...
  s.getRange(cursor, 2, datos.length, 4).setValues(datos)
   .setFontFamily(F.body).setFontSize(11).setFontColor(C.textMain).setVerticalAlignment('middle').setBackground(C.bg);
  for (let i = 0; i < datos.length; i++) {
    s.getRange(cursor + i, 2, 1, 4).setBorder(null, null, true, null, false, false, C.lineHair, SpreadsheetApp.BorderStyle.SOLID);
    s.setRowHeight(cursor + i, 28);
  }
